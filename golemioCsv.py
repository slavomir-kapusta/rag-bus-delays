...
def run_api_query(csv_filename):
    ...
    if max_old_time:
        print(f"Bude se filtrovat od času: {max_old_time.strftime('%d.%m.%Y %H:%M:%S')}")
        ...
        for feature in features:
            ...
            if delay_min >= MIN_DELAY:
                ...
                doc_text = (f"Dne {den_str} měla linka {linka} / {sequence} zpoždění {delay_min} minut "
                            ...
                            f"vozidlo: {vehicle}, "
                            f"zpoždění: {delay_min}, "
                            f"zastávka: {zastavka}, "
                            f"lat: {lat}, "
                            f"lon: {lon}, "
                            f"směr: {smer}, "
                            f"kategorie: provoz, "
                            f"příjezd: {prijezd_str}")
...
if __name__ == "__main__":
    ...
    with open(master_csv_filename, mode="w", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writerow(["ID Záznamu", "Timestamp", "Den", "Čas příjezdu", "Linka", "Oběh", "Vozidlo","Zpoždění (min)", "Zastávka", "Směr", "Zeměpisná šířka (Lat)", "Zeměpisná délka (Lon)"])
