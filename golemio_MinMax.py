...
SLEEP_TIME = 240 # 240 # (   240sec ---= 4 minuty )  
OVERLAP_TIME = 120 # 240 # (   120sec ---= 2 minuty )  
...
# Původní kód pro dnešek:
# today_prefix = datetime.now().strftime("%Y%m%d")

# Nový kód pro včerejšek:
yesterday_prefix = (datetime.now() - timedelta(days=1)).strftime("%Y%m%d")

# --- FUNKCE: Tisk minimálních a maximálních časů ze všech včerejších souborů ---
def print_prev_min_max_times(data_dir="data"):
    """
    Najde všechny dnešní JSONy a vrátí:
    1. Minimální a maximální zaznamenané arrival_time (max_time,max_time) ze všech včerejších souborů.
    """
    print(f"Čtu soubory pro včerejší den a hledám minimální a maximální časy příjezdů...")
    if not os.path.exists(data_dir):
        return None, {}

    yesterday_prefix = (datetime.now() - timedelta(days=1)).strftime("%Y%m%d")
    old_files = sorted(glob.glob(os.path.join(data_dir, f"{yesterday_prefix}_*.json")))

    if not old_files:
        return None, {}

    print(f"Čtu {len(old_files)} souborů pro včerejší den...")

    # Cyklus přes všechny včerejší soubory
    for current_file in old_files:
        try:
            with open(current_file, "r", encoding="utf-8") as f:
                ...
                for item in data:
                    try:
                        ...
                        if den and prijezd and linka is not None:
                            # 1. Kontrola a aktualizace minimálního a maximálního času
                ...
                print(f"{current_file} min: {min_time} max: {max_time}")
    
        except Exception as e:
            print(f"Varování: Chyba při čtení historického souboru {current_file}: {e}")
    
    # Odečtení OVERLAP_TIME od max_time
    
    print(f"Zpracováno {len(old_files)} souborů.") 

    return

# --- FUNKCE: Tisk minimálních a maximálních časů ze všech dnešních souborů ---
def print_min_max_times(data_dir="data"):
    """
    Najde všechny dnešní JSONy a vrátí:
    1. Minimální a maximální zaznamenané arrival_time (max_time,max_time) ze všech dnešních souborů.
    """
    print(f"Čtu soubory pro dnešní den a hledám minimální a maximální časy příjezdů...")
    if not os.path.exists(data_dir):
        return None, {}
    ...
    print(f"Čtu {len(files)} souborů pro dnešní den...")
    
    # Cyklus přes všechny dnešní soubory
    for current_file in files:
        try:
            with open(current_file, "r", encoding="utf-8") as f:
                max_time = None
                min_time = datetime.now()
                if current_file == "data\\20260317_144534.json":
                    print(f"Zpracovávám soubor: {current_file}")
                data = json.load(f)
                for item in data:
                    try:
                        meta = item.get("metadata", {})
    ...
    print(f"Zpracováno {len(files)} souborů.") 
...
def load_last_run_data(data_dir="data"):
            ...
            for item in data:
                try:
                    ...
                    if den and prijezd and linka is not None:
                        ...
                        key = (linka, obeh, vozidlo, den, prijezd)
    ...
    # Subtract 120 seconds from max_time
    orig_max_time = max_time
    max_time = max_time - timedelta(seconds=OVERLAP_TIME)

    #print(f"{latest_file} min: {min_time} max: {orig_max_time} Adjusted time: {max_time}")
    print(f"{latest_file} min: {min_time} max: {orig_max_time}")
...
def run_api_query(csv_filename):
    ...
    # --- výpis minimálních a maximálních časů
    #print_prev_min_max_times("data")
    print_min_max_times("data")
    ...
    try:
        ...
        for feature in features:
            ...
            #print(f"================================")
            #print(f"linka {linka_str} / {sequence_str}  vehicle {vehicle} ")
            ...
            if delay_min >= MIN_DELAY:
                ...
                #bad sekvence = trip_data.get('sequence_id') or " "
                ...
                try:
                    ...
                    record_key = (linka, sequence, vehicle, den_str, prijezd_str)
                ...
                #if linka==177:
                #    print(f"{den_str} {prijezd_str} bus {linka}/{sequence} zpoždění {delay_min} min u zastávky {zastavka} (směr {smer}) ")

                now_str = datetime.now().strftime("%Y%m%d_%H%M%S")
                record_id = f"{now_str}_L{linka}"

                doc_text = (f"Dne {den_str} měla linka {linka} / {sequence} zpoždění {delay_min} minut "
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            #print(f"{datetime.now().strftime('%H:%M')} - Čekám " + str(SLEEP_TIME / 60) + " minuty na další dotaz...\n")
